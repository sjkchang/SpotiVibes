...
export async function getTopItems(type="tracks", timeRange="medium_term", limit=20, offset=0) {
  let accessToken = localStorage.getItem('access_token');
...
export async function getPlaylists(limit=20, offset=0) {
  ...
  const response = await fetch('https://api.spotify.com/v1/me/playlists/?' + args, {
    headers: {
      Authorization: 'Bearer ' + accessToken
    },
  });
